...
昨日、ジオメトリから処理する部分を書いたので、色の作り方のテスト。  

### フルカラーボクセル環境の準備  

メッシュの頂点に色を入力すると、頂点と頂点の間は線形補完されて色がつくのがわかった。うまくメッシュを実装すれば、距離などの条件に応じた色を少ない格子点で計算し、面に反映できそう。四角形の対角の片方で三角形に分割したメッシュであると、分割の仕方による異方性的なものが出たのでこの辺はこれから詰める。  
